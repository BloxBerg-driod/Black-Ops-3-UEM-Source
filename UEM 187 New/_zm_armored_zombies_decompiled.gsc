...
function private function_bd226691()
{
	...
	{
		if(level.var_317e7556 <= 1 && level.var_c806dee2 == 0)
		{
			self.no_powerups = 1;
			...
			if(isalive(self))
			{
				...
				if(isdefined(valid_drop) && valid_drop)
				{
					var_162e1c19 setmodel("zmu_halloween_pumpkin_teddy");
					final_pos = zm_weapons::function_a2b97522(self, 50, var_162e1c19, 1);
					var_8f7442a5 = util::ground_position(final_pos, 500, 25);
					var_162e1c19 zm_utility::fake_physicslaunch(var_8f7442a5, randomintrange(50, 100));
...
function function_34cead27()
{
	...
	while(isdefined(self))
	{
		self.trigger_use waittill("trigger", player);
		self thread function_916e719a(player);
		if(player.pers["hc_trickortreat"] < 151)
		{
			player.pers["hc_trickortreat"]++;
			if(player.pers["hc_trickortreat"] == 150)
			{
				player.pers["hc_trickortreat"] = 150;
				player.pers["halloween_pumpkin_hat"] = 1;
				player thread namespace_97ac1184::function_ec5385f9();
			}
		}
		self notify("hash_690bf263");
	}
...
function function_916e719a(player)
{
	...
	switch(index)
	{
		...
		case 4:
		{
			...
			player notify("hash_79ef118b", "christmas_present", XP);
		...
		case 6:
		{
			...
			{
				playsoundatposition("halloween_laugh", self.origin);
				player thread function_9d27c73a();
				player luinotifyevent(&"spx_milestone_notification", 2, &"ZM_MINECRAFT_PICKUP_UI", 7);
			}
			else
			{
				self thread function_916e719a(player);
...
function tomb_trigger_update_message(func_per_player_msg)
{
	...
	return;
	~self.stub.playertrigger[n_player].stored_hint_string;
}
...
{
	if(!zm_utility::is_player_valid(player) || player.is_drinking > 0 || !player zm_magicbox::can_buy_weapon())
	{
