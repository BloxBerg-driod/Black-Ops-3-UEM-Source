...
function function_ab74af48(player)
{
	...
	else
	{
		...
		wait(0.8);
		var_ecd7621f = util::spawn_model("zmu_nuke_barebones", self.origin, self.angles - VectorScale((0, 1, 0), 180));
...
function grab_check(player, random_perk)
{
	...
	while(!perk_is_bought)
	{
		...
		if(e_triggerer == player)
		{
			...
			if(player hasperk(random_perk))
			{
				iprintln("^8[Wunderfizz] Softlock Prevented | Cannot take perk");
				continue;
			}
...
function player_has_all_available_perks()
{
	...
	{
		if(!(isdefined(level.var_c0bbcc46[level._custom_perks[i]]) && level.var_c0bbcc46[level._custom_perks[i]]))
		{
			continue;
		}
